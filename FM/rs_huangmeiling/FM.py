...
def decode_csv(line, feature_size):
    ...
    dense_vector = tf.reshape(tf.string_to_number(columns.values[1:feature_size + 1], out_type=tf.float32),
                              [feature_size])
    ...
    def train(self, batch_data):
        ...
        with tf.name_scope('input_data'):
            ...
            dense_vector = tf.reshape(batch_data['dense_vector'],
                                      shape=[-1, feature_size, 1])  # None * feature_size * 1
        ...
        with tf.name_scope("FM_Comput_Score"):
            ...
            # ---------- score ----------  
            score_1 = tf.nn.sigmoid(Y_Out, name='score')
            score = tf.reshape(score_1, shape=[-1, 1])
        ...
        """6 返回结果 """
        return Y_Out, score, regularization, loss, auc, train_step, w_zero_ratio, w_avg, v_zero_ratio, v_avg, labels, score_1, summary_op
...
# 测试数据
filenames = r'E:\Recommendation_System_practice\FM\rs_huangmeiling\data'
data_type = 'csv'
feature_size = 530
batch_size = 64
num_epochs = 200
...
reg_type = 'l2_reg'
reg_param = 0.000
log_path = r"E:\Recommendation_System_practice\FM\rs_huangmeiling\FM_Cross_entropy_L2_0_20180816_01"

# 开始训练
bea_model = FM(feature_size, fm_v_size, loss_fuc, train_optimizer, learning_rate, reg_type, reg_param)
Y_Out, score, regularization, loss, auc, train_step, w_zero_ratio, w_avg, v_zero_ratio, v_avg, labels, score_1, summary_op = bea_model.train(
    next_element)
